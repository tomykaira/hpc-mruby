-- http://d.hatena.ne.jp/mokehehe/20090214
...
import qualified Data.ByteString as BS (putStr, pack)
...
-- main 
main = do putStrLn "P6"                                                 -- ppm-header
          putStrLn $ show (floor width) ++ " " ++ show (floor height)   -- ppm-header
          putStrLn "255"                                                -- ppm-header
          let img = evalState (render init_scene width height nsubsamples nao_samples)  -- rendering-image
                    (123456789, 362436069, 521288629, 88675123)
          -- forM_ img (\(r,g,b) -> putStrLn $ show r ++ " " ++ show g ++ " " ++ show b) -- output(ascii)
          BS.putStr . BS.pack . map fromIntegral $ foldr (\(r, g, b) acc -> r:g:b:acc) [] img -- output(binary)
